...
engine::registry &R_Type::Rtype::getRegistry() { return _registry; }
    ...
    if (auto pkt_opt = _client->receive(_sender)) {
      ...
      if (recvHdr.type == CONNECT_ACK && payload.size() >= sizeof(ConnectAck)) {
        ...
        if (_player >= positions.size())
          positions.insert_at(_player, component::position{100.f, 100.f});
        ...
        if (_player >= drawables.size()) {
          drawables.insert_at(_player,
                              component::drawable{_playerTexture->texture,
                                                  _playerTexture->playerRect});
        }
