...
{
    if (moverIdx >= collisions.size() || !collisions[moverIdx] || !positions[moverIdx] || !hitboxes[moverIdx] ||
        blockerIdx >= positions.size() || !positions[blockerIdx] || !hitboxes[blockerIdx])
        return;
    ...
    if (overlapX < overlapY) {
        if (acx < bcx) moverPos.x -= overlapX; else moverPos.x += overlapX;
        if (moverIdx < velocities.size() && velocities[moverIdx]) velocities[moverIdx]->vx = 0.f;
    } else {
        if (acy < bcy) moverPos.y -= overlapY; else moverPos.y += overlapY;
        if (moverIdx < velocities.size() && velocities[moverIdx]) velocities[moverIdx]->vy = 0.f;
    }
}
