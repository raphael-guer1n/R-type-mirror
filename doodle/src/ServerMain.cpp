...
namespace {
    ...
    const float SPAWN_AHEAD = 300.0f;
        while (playerY < minPlatformY + SPAWN_AHEAD) {
            ...
            reg.add_component(pe, component::hitbox{PLATFORM_W_C, PLATFORM_H_C});
            int kd = kindDist(rng);
            uint8_t pkind = 0;
            if (kd <= 5) {
                pkind = 0;
            ...
            if ((pkind == 0 || pkind == 1) && enemySpawnRoll(rng) == 0) {
                ...
                if (posAll[pe] && posAll[pe].has_value() && hbAll[pe] && hbAll[pe].has_value()) {
                    ...
                    float ex = enemyOffset01(rng) * (SCREEN_W - ENEMY_W_C);
                    float ey = pp.y - ENEMY_H_C - (10.0f + enemyOffset01(rng) * 40.0f);
                    ...
                    reg.add_component(en, component::hitbox{ ENEMY_W_C, ENEMY_H_C });
    ...
    {
        const float PRUNE_BELOW = SCREEN_H * 0.6f;
        auto &posArr = reg.get_components<component::position>();
        auto &playerPosArr = reg.get_components<component::position>();
        if (!(player < playerPosArr.size() && playerPosArr[player] && playerPosArr[player].has_value())) {
            return;
        }
        float playerY = playerPosArr[player].value().y;
    ...
    void resolvePlayerPlatformCollisions(registry &reg, entity_t player, float fixedDt, float &outLastSafeY)
    {
        ...
        if (!(player < poss.size() && poss[player] && player < vels.size() && vels[player] && player < hbs.size() && hbs[player])) {
            return;
        }
        ...
        for (size_t e = 0; e < poss.size(); ++e) {
            ...
            if (!(e < plats.size() && plats[e] && plats[e].has_value())) {
                continue;
            }
            ...
            if (vels[player].value().vy > 0 && overlapX && prevBottom <= platTop + 5.0f && curBottom > platTop) {
                ...
                uint8_t pkind = plats[e].value().kind;
                outLastSafeY = platTop;
                if (pkind == 3) {
    ...
    void resolvePlayerEnemyCollisions(registry &reg, std::vector<entity_t> &enemies, entity_t player, float playerSpawnX, float playerSpawnY, bool &gameStarted, float fixedDt)
    {
        ...
        for (auto en : enemies) {
            ...
            if (!(en < hbs.size() && hbs[en] && hbs[en].has_value())) {
                continue;
            }
            ...
            if (stomp) {
                reg.kill_entity(en);
                vels[player].value().vy = PLAYER_JUMP_V * STOMP_MULT;
                ...
                for (size_t i = 0; i < projArrClr.size(); ++i) {
                    if (projArrClr[i] && projArrClr[i].has_value()) {
    ...
    for (auto en : enemies) {
        if (!(en < posAll.size() && posAll[en] && posAll[en].has_value())) {
            continue;
        }
        if (!(en < hbAll.size() && hbAll[en] && hbAll[en].has_value())) {
            continue;
        }
        if (!(en < velAll.size() && velAll[en] && velAll[en].has_value())) {
            continue;
        }
    ...
    void resolveProjectileEnemyHits(registry &reg, std::vector<entity_t> &enemies)
    {
        ...
        for (auto en : enemies) {
            ...
            for (size_t i = 0; i < projAll.size(); ++i) {
                if (!projAll[i] || !projAll[i].has_value()) {
                    continue;
                }
                entity_t projE = reg.entity_from_index(i);
                if (!(posAll[projE] && hbAll[projE])) {
                    continue;
                }
    ...
    void cullProjectiles(registry &reg, entity_t player)
    {
        ...
        for (size_t i = 0; i < projArr.size(); ++i) {
            if (!projArr[i] || !projArr[i].has_value()) {
                continue;
            }
            ...
            if (i < posArr.size() && posArr[i] && posArr[i].has_value()) {
                if (player < posArr.size() && posArr[player] && posArr[player].has_value()) {
                    float pY = posArr[player].value().y;
                    const float TOP_MARGIN = 1500.0f;
                    const float BOTTOM_MARGIN = 2500.0f;
                    float y = posArr[i].value().y;
                    if (y < pY - TOP_MARGIN || y > pY + BOTTOM_MARGIN) {
                        toKillProj.push_back(reg.entity_from_index(i));
                    }
                }
                else {
                    if (posArr[i].value().y < -10000.0f || posArr[i].value().y > 10000.0f) {
                        toKillProj.push_back(reg.entity_from_index(i));
                    }
                }
            }
        }
        for (auto e : toKillProj) {
            reg.kill_entity(e);
        }
    }
    ...
    std::cout << "Doodle server listening on port " << port << std::endl;
        ...
        while (running) {
            ...
            while (accumulator >= fixedDt) {
                ...
                if (gameStarted) {
                    clampEnemiesToBounds(reg, enemies);
                }
